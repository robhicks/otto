...
async fn mcp_git_commits_and_stays_gated() {
    let bin = escargot::CargoBuild::new()
        .package("otto-mcp-git")
        .bin("mcp-git")
        .run()
        .expect("build mcp-git")
        .path()
        .to_path_buf();
    ...
    let (_conn, tools) = mcp_connect_git(bin.to_str().unwrap(), dir.path())
        .await
        .expect("connect to mcp-git");
    ...
    registry
        .call("git.add", json!({ "paths": ["a.txt"] }))
        .await
        .unwrap();
    let out = registry
        .call("git.commit", json!({ "message": "via mcp" }))
        .await
        .unwrap();
    ...
    assert!(
        denied
            .expect_err("sensitive add must be denied")
            .to_string()
            .contains("denied")
    );
}
