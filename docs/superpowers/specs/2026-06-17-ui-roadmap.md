...
| **G** ✅ | **Tauri desktop wrapper** *(shipped 2026-07-01 — [design](2026-07-01-ui-tauri-desktop-wrapper-design.md) · [plan](../plans/2026-07-01-ui-tauri-desktop-wrapper.md))* | A native desktop app: pick a workspace folder, auto-launch a local `otto serve` as a Tauri sidecar, auto-connect the existing `ui/` client — no manual URL/token setup. The originally-envisioned EMBEDDED (v1 default) mode from `ARCHITECTURE.md`; A–F are all the SERVED (v2) mode. | **None.** Fully additive: no `protocol`/engine-core changes; `ui/` gains one pure query-param-parsing helper (`ui/src/url.rs`) and an auto-connect check in `app.rs`. New sibling crate `desktop/` (workspace-excluded like `ui/`) owns the Tauri/sidecar logic and depends on zero `otto-*` crates. |
...
- **G** comes after F, appended once the browser client was feature-complete: it wraps the
  proven `ui/dist` build unmodified rather than co-developing alongside it, and needs no new
  protocol/engine surface — the lowest-risk possible slice to close out the roadmap with.
...
- **`desktop/`** (sub-project G) — standalone Tauri 2 crate, also **not** a workspace member.
  Depends on zero `otto-*` crates (spawns the compiled `otto` binary as an external sidecar
  process instead of linking anything); its `frontendDist` points at `../ui/dist` unmodified.
