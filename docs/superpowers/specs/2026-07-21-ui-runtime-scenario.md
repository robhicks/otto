...
- **Workspace fixture:** a fixed, disposable git repo created fresh per run from the
  ...
  exist on disk but be omitted entirely from the `POST /workspace` listing — confirms the
  sensitive-path floor filters it out server-side before the listing ever reaches the
  client, rather than listing it and denying the open), and a `cargo test`-runnable test the
...
| 6 | C | Expand workspace tree | The nested `src/util/` directory expands/collapses; `.env` never appears as a node in the rendered tree at all — the sensitive-path floor (`crates/engine/src/service.rs`'s `POST /workspace` `List` handler) filters it out of the listing server-side, so the UI has nothing to render or deny for it, not a listed-then-denied entry |
...
| 6 | Playwright clicks the `src/util` tree node, asserts it expands (child node visible) then collapses on a second click; asserts `.env` does not appear as a node anywhere in the rendered tree (queries the DOM for its absence, not for a denial state) | The workspace tree is populated via the bearer-authed `POST /workspace` RPC, which is externally observable: capture the request/response (server log or a proxying layer) and assert the returned path list omits `.env` entirely, confirming the sensitive-path floor filtered it before the response ever left the server. **NOT-VERIFIABLE (desktop)** applies only to the purely in-window `src/util` expand/collapse animation state, which has no external artifact — not to the `.env`-omission check, which the RPC payload itself proves |
