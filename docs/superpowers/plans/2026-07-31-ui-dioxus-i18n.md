...
> **Status:** SHIPPED — all 6 tasks landed in [#118](https://github.com/robhicks/otto/pull/118),
> together with the review round that followed it (the Traditional-Chinese resolution fix, the
> `locales!` macro, the brace-integrity guardrails, the runtime locale-switch test, the token
> redaction, and the cross-target `document.lang` write).
...
| `ui-dioxus/src/app.rs` | **Modify.** Provide the locale signal; localize three buttons + the validation message; startup `document.lang` effect (cross-target). |
