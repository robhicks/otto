...
In `ui-dioxus/Cargo.toml`, extend the `web` feature list with `dep:js-sys` — `Navigator::languages()`
returns a `js_sys::Array`, and `js-sys` is currently only a transitive lock entry (nothing under
`src/` uses it today):

```toml
web = ["dioxus/web", "dep:web-sys", "dep:js-sys", "dep:gloo-net", "dep:wasm-bindgen", "dep:getrandom", "uuid/js"]
```

and add the dependency itself next to `web-sys`:

```toml
# `Navigator::languages()` returns a js_sys::Array (i18n locale detection). Already present
# transitively via wasm-bindgen, so this is a manifest edit with no bundle cost — but it must be a
# DIRECT dependency to be importable.
js-sys = { version = "0.3", optional = true }
```

Then extend the `desktop` feature list to include the two new optional deps (add them to the
...
# not write settings into the user's workspace — the engine indexes and edits that tree. Version
# matched to the workspace's own `dirs` (crates/engine/Cargo.toml).
...
- [ ] **Step 6: Wire the modules and add `initial_locale`**

In `ui-dioxus/src/i18n/mod.rs`, add below `mod catalog;`:
...
Also update `ApprovalPanel`'s and `StatusLine`'s call sites of the changed signatures if the
compiler flags them here — Task 5 owns their copy, but this task owns keeping them compiling. Pass
`use_locale()` from within each component.
...
Add to `ui-dioxus/src/components/language_picker.rs` (test module first):
...
In `ui-dioxus/src/components/mod.rs`, add `mod language_picker;` (alphabetically, after
`mod file_tree;`) and `pub use language_picker::{set_document_lang, LanguagePicker};`.
...
In `ui-dioxus/src/app.rs`, at the TOP of the component body alongside the other `use_signal` calls
(hooks are positional — this must be unconditional and outside every `#[cfg]` block):
...
Add to `ui-dioxus/src/components/status_line.rs`'s test module (create the module if absent):
...
mod tests {
    ...
    #[test]
    fn status_strip_follows_the_active_locale() {
        assert!(render(Locale::En).contains("disconnected"));
        assert!(render(Locale::De).contains("getrennt"));
        assert!(render(Locale::ZhHans).contains("已断开"));
    }

    #[test]
    fn status_strip_renders_without_a_provider() {
        // Same provider-less guarantee as `editor/dirty.rs`'s render tests.
        #[component]
        fn Bare() -> Element {
            let conn = use_signal(|| ConnState::Disconnected);
            let last_seq = use_signal(|| None::<u64>);
...
Run: `cd ui-dioxus && cargo test --features desktop status_line`
Expected: FAIL — the strip still renders English regardless of locale.
...
- **`app.rs`** — add `let locale = use_locale();` at the top of the body (with the other hooks),
  then `"Refresh files"` → `{t(locale, Msg::RefreshFiles)}`, `"Promote to remote"` →
  `Msg::PromoteToRemote`, `"Demote to local"` → `Msg::DemoteToLocal`.
- **`desktop_boot.rs:120`** — `.set_title("Choose a workspace folder")` →
  `.set_title(t(crate::i18n::initial_locale(), crate::i18n::Msg::ChooseWorkspaceFolder))`.
...
Run this sweep and read the output — every hit must be a §2 exclusion (a glyph, a CSS class, a
protocol identifier, a passthrough diagnostic, a URL example, or a number format):

```bash
cd ui-dioxus && grep -rn '"[A-Za-z][A-Za-z ]\{3,\}"' src/ --include='*.rs' | grep -v '^src/i18n/' | grep -v '#\[cfg' | grep -v 'mod tests'
```
Expected: only exclusions. Any authored user-facing sentence still present is a miss — fix it before
committing.

- [ ] **Step 7: Verify all three build configurations**
...
- [ ] **Step 4: Run the bundle-trust guards (out-of-band surface)**
...
- [ ] **Step 5: Run clippy on the UI crate**
