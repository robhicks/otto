...
```
Expected: clean. Note this only has to hold **at the end**: Tasks 1 and 2 commit `pub` items that
nothing consumes yet, and `pub` does not suppress `dead_code` in a binary crate, so those two
commits legitimately warn. By this task everything is wired and the warnings are gone — if any
remain, something from Tasks 1–2 was never used and should be deleted or wired.
