...
```rust
        // Startup only; the picker applies it again on every change.
...
In each component, call `let locale = use_locale();` unconditionally at the top of the body (above
any early return), then replace each literal:
...
- **`approval_panel.rs`** — the header `"approval needed: {path.display()}"` →
  `{tf(locale, Msg::ApprovalNeeded, &[("path", &path.display().to_string())])}`, and
  `"Approve"`/`"Reject"` → `Msg::Approve`/`Msg::Reject`. Note `use_locale()` must be called BEFORE
  the `let Some(...) = pending.read().clone() else { return rsx! {} };` early return.
  Its `diff_lines` call gains `locale` as its first argument.
- **`status_line.rs`** — `status_label(&c)` → `status_label(locale, &c)`; `"seq {seq}"` →
  `{tf(locale, Msg::SeqLabel, &[("seq", &seq)])}`; `capability_segments(m)` →
  `capability_segments(locale, m)`; `format_meter(i, o)` → `format_meter(locale, i, o)`.
...
`.prompt`/`.conn-form` (`:68`) are `display: flex` with `flex` sizing on the inputs — so nothing
clips. Run:
