...
- **Remote slot** — one of four providers, selected when its API key is present. Precedence when several keys are set is `ANTHROPIC_API_KEY` > `OPENAI_API_KEY` > `GEMINI_API_KEY` > `DEEPSEEK_API_KEY`; set `OTTO_REMOTE_PROVIDER=anthropic|openai|gemini|deepseek` to pick explicitly (a named provider whose key is absent falls back to offline, never to another key). Any selected remote enables the `BrainBlendRouter`; absent every key/selector, a `SingleProviderRouter` over the local slot is used (the offline-deterministic default).
  ...
  - Validating the destination is not sufficient on its own, so the provider's HTTP client also **disables redirects** (reqwest strips `Authorization` on a host/port change but *not* on an https→http scheme downgrade, and a 307/308 would re-POST the prompt body to an unvalidated host) and **disables the system proxy for `http` bases** (`HTTP_PROXY`/`ALL_PROXY` are honored with no loopback exemption, which would otherwise send the cleartext request across the network and void the loopback carve-out entirely). `https` bases keep proxy support — the proxy sees only a CONNECT tunnel.
