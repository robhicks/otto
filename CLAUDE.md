...
`lsp.diagnostics`/`lsp.definition`/`lsp.references`/`lsp.hover` to `rust-analyzer` — diagnostics
are debounced until the publish stream quiesces, since rust-analyzer publishes an empty
pre-analysis set first; additive and Rust-only in v1, multi-language dispatch deferred) are all
real rmcp stdio servers. The engine's MCP client adapter
...
| `mcp-lsp` | rmcp stdio server (`mcp-lsp <root>`): bridges `lsp.diagnostics`/`lsp.definition`/`lsp.references`/`lsp.hover` to a `rust-analyzer` child over a hand-rolled LSP-over-stdio client (Content-Length framing, request/response dispatch, generation-tracked `publishDiagnostics` cache with a quiescence debounce — rust-analyzer publishes an empty pre-analysis set before the real one). Positions are 1-based in/out. Documents sync via full-text `didOpen`/`didChange` from a path-contained `LocalWorkspace`. Rust-only in v1; the root is canonicalized at startup; registered additively (absent when `rust-analyzer` is missing); `OTTO_RUST_ANALYZER_BIN`/`OTTO_MCP_LSP_BIN`/`OTTO_MCP_LSP_DIAG_TIMEOUT_MS` env overrides. |
