...
| `retrieval` | Persistent inverted index behind the `Retriever` seam (defined in `engine-core`). `IndexedRetriever` keeps a standalone sqlite index (token→file postings) in the OS cache dir keyed by workspace root, refreshed stat-incrementally (mtime+size) and atomically per file, and scores content for every indexed file — removing the ContextFinder's per-turn read budget. Mirrors the gate's sensitive-path floor (secrets never indexed; the walk is the sole defense since the index reads files directly). Depends inward on `engine-core`. |
...
pipeline — the deterministic offline path — when absent.
